...
pub fn getNavVAddr(
    ...
    const this_sym_index = try self.getOrCreateMetadataForNav(elf_file, nav);
...
pub fn freeNav(self: *ZigObject, elf_file: *Elf, nav_index: InternPool.Nav.Index) void {
    const gpa = elf_file.base.comp.gpa;
    log.debug("freeNav 0x{}", .{nav_index});
...
fn updateNavCode(
    ...
    log.debug("updateNavCode {}", .{nav.fqn.fmt(ip)});
...
fn updateTlv(
    ...
    log.debug("updateTlv {}", .{nav.fqn.fmt(ip)});
...
pub fn updateNavLineNumber(
    ...
    log.debug("updateNavLineNumber {} 0x{x}", .{ nav.fqn.fmt(ip), nav_index });
